...
public class Week06TicTacToeLab {
	...
	public static void main(String[] args) {
		...
		System.out.println("We are about to about Tic Tac Toe.");
		...
		turns = new Turns(playerX, playerO);		

		boolean isWinner = false;
		int currentPlayer = 0;
		
		while(!isWinner && turns.getTurn() <= 9) {
			System.out.println("Current board is: ");
			board.display();

			currentPlayer = turns.next();
			makeMove(currentPlayer);
			isWinner = board.checkWinStatus();	
		}
		...
		if(isWinner) {
			System.out.println(turns.getPlayer(currentPlayer) + " has won!");
		} else {
			System.out.println("Looks like it was a draw - no winner!");
		}
			
		
	}
	
	public static void makeMove(int currentPlayer) {
		boolean inProgress = true;
		
		while(inProgress) {
			try {
				int location = scanner.nextInt();
				board.placePiece(location, currentPlayer);
